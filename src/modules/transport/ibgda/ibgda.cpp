...
static int ibgda_setup_backup_rc_endpoints(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    // Get backup device and port from mapping
    int backup_dev_id = ibgda_state->backup_dev_ids[device_idx];
    int backup_port_id = ibgda_state->backup_port_ids[device_idx];
    
    if (backup_dev_id == -1 || backup_port_id == -1) {
        INFO(ibgda_state->log_level, "No backup device/port available for device_idx=%d, skipping backup QP creation", device_idx);
        return NVSHMEMX_SUCCESS;
    }
    ...
    device->rc.backup_port_id = backup_port_id;
...
static int ibgda_setup_gpu_state(nvshmem_transport_t t) {
    ...
    if (num_backup_rc_handles > 0) {
        ...
        for (int i = 0; i < num_backup_rc_handles / n_devs_selected; ++i) {
            ...
            for (int j = 0; j < n_devs_selected; j++) {
                ...
                backup_rc_h[arr_idx].rx_wq.cq = &backup_cq_d[backup_cq_idx];

                ibgda_get_device_cq(&backup_cq_h[backup_cq_idx], device->rc.backup_eps[i]->recv_cq);
                backup_cq_h[backup_cq_idx].resv_head = (uint64_t *)(base_mvars_d_addr + rx_resv_head_offset);
                backup_cq_h[backup_cq_idx].cons_idx = (uint64_t *)(base_mvars_d_addr + rx_cons_offset);
                backup_cq_h[backup_cq_idx].qpn = backup_rc_h[arr_idx].qpn;
                backup_cq_h[backup_cq_idx].qp_type = backup_rc_h[arr_idx].qp_type;
                ++backup_cq_idx;
            }
    ...
    status = ibgda_init_health_management_arrays(
        ...
        reinterpret_cast<ibgda_qp_health_status_t **>(
            &ibgda_device_state_h->globalmem.rc_health_status),
        &ibgda_device_state_h->globalmem.rc_failure_count,
        &ibgda_device_state_h->globalmem.rc_last_check_time,
        &ibgda_device_state_h->globalmem.rc_switch_time);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemt_ibgda_finalize(nvshmem_transport_t transport) {
    ...
    if (ibgda_device_state_h) {
        ...
        // Free backup RC related device memory
        if (ibgda_device_state_h->globalmem.backup_rcs) 
            cudaFree(ibgda_device_state_h->globalmem.backup_rcs);
        if (ibgda_device_state_h->globalmem.backup_cqs) 
            cudaFree(ibgda_device_state_h->globalmem.backup_cqs);
